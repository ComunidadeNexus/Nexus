...
const emptyPkg = {
  name: "", coins: 100, bonus_coins: 0, price: 9.90, currency: "BRL",
...
const AdminCoins = () => {
  ...
  useEffect(() => { fetchPackages(); fetchTransactions(); }, []);
  ...
  const fetchTransactions = async () => {
    setLoadingTxns(true);
    const { data } = await supabase.from("coin_transactions").select("*").order("created_at", { ascending: false }).limit(50);
    setTransactions(data || []);
    setLoadingTxns(false);
  };

  const openCreate = () => { setEditTarget(null); setForm({ ...emptyPkg }); setDialogOpen(true); };
  const openEdit = (p: CoinPackage) => {
    setEditTarget(p);
    setForm({ name: p.name, coins: p.coins, bonus_coins: p.bonus_coins, price: p.price, currency: p.currency, description: p.description || "", is_active: p.is_active, is_popular: p.is_popular });
    setDialogOpen(true);
  };
  ...
  const handleCredit = async () => {
    if (!creditUserId || !creditAmount) { toast({ title: "Preencha todos os campos", variant: "destructive" }); return; }
    await creditCoins(creditUserId, Number(creditAmount), creditDesc);
    fetchTransactions();
    setCreditDialog(false);
    setCreditUserId(""); setCreditAmount("50"); setCreditDesc("");
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={() => setCreditDialog(true)} variant="outline" className="border-white/10 hover:bg-white/10">
      ...
      {activeTab === "packages" && (
        loadingPkgs ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {Array(4).fill(0).map((_, i) => <Skeleton key={i} className="h-44 rounded-2xl" />)}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {packages.map(pkg => (
              <div key={pkg.id} className="relative p-5 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/8 transition-all">
                {pkg.is_popular && (
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2">
                    <Badge className="bg-gradient-to-r from-violet-600 to-purple-600 text-white border-0 text-xs">⭐ Popular</Badge>
                  </div>
                )}
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-amber-400 mt-1">
                      {pkg.coins}
                      {pkg.bonus_coins > 0 && <span className="text-sm text-emerald-400 ml-1">+{pkg.bonus_coins}</span>}
                ...
                <div className="flex gap-1.5 mb-3">
                  <Badge className={`text-xs ${pkg.is_active ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : "bg-red-500/20 text-red-400 border-red-500/30"}`}>
                    {pkg.is_active ? "Ativo" : "Inativo"}
                  </Badge>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="flex-1 border-white/10 hover:bg-white/10 text-xs" onClick={() => openEdit(pkg)}>
                    <Pencil className="w-3.5 h-3.5 mr-1" /> Editar
                  </Button>
                  <Button size="sm" variant="outline" className="border-white/10 hover:bg-white/10 text-xs" onClick={() => toggleActive(pkg.id, pkg.is_active)}>
                    {pkg.is_active ? "Desativar" : "Ativar"}
                  </Button>
                  <Button size="sm" variant="outline" className="border-red-500/20 hover:bg-red-500/10" onClick={() => setDeleteTarget(pkg.id)}>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loadingTxns ? Array(6).fill(0).map((_, i) => (
                  <tr key={i} className="border-b border-white/5">
                    {Array(4).fill(0).map((_, j) => <td key={j} className="px-4 py-3"><Skeleton className="h-8 rounded" /></td>)}
                  </tr>
                )) : transactions.length === 0 ? (
                  <tr><td colSpan={4} className="text-center text-muted-foreground py-12">Nenhuma transação</td></tr>
                ) : transactions.map(t => (
                  <tr key={t.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="px-4 py-3">
                      <Badge className="text-xs bg-white/10 text-foreground border-white/10">{t.type}</Badge>
                    </td>
                    <td className="px-4 py-3">
        ...
        <DialogContent className="max-w-md bg-background/95 backdrop-blur border-white/10">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1.5">
                <Label>Coins</Label>
                <Input type="number" value={form.coins} onChange={e => setForm(f => ({ ...f, coins: Number(e.target.value) }))} className="bg-white/5 border-white/10" />
              </div>
              <div className="space-y-1.5">
                <Label>Bônus</Label>
                <Input type="number" value={form.bonus_coins} onChange={e => setForm(f => ({ ...f, bonus_coins: Number(e.target.value) }))} className="bg-white/5 border-white/10" />
              </div>
              <div className="space-y-1.5">
                <Label>Preço (R$)</Label>
                <Input type="number" step="0.01" value={form.price} onChange={e => setForm(f => ({ ...f, price: Number(e.target.value) }))} className="bg-white/5 border-white/10" />
              </div>
            </div>
            <div className="space-y-1.5">
        ...
        <DialogContent className="bg-background/95 backdrop-blur border-white/10">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input value={creditUserId} onChange={e => setCreditUserId(e.target.value)} className="bg-white/5 border-white/10 font-mono text-xs" placeholder="UUID do usuário..." />
            </div>
            <div className="space-y-1.5">
              <Label>Quantidade de Coins</Label>
              <Input type="number" value={creditAmount} onChange={e => setCreditAmount(e.target.value)} className="bg-white/5 border-white/10" />
            </div>
            <div className="space-y-1.5">
              <Label>Motivo (opcional)</Label>
              <Input value={creditDesc} onChange={e => setCreditDesc(e.target.value)} className="bg-white/5 border-white/10" placeholder="Descrição..." />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setCreditDialog(false)}>Cancelar</Button>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={open => !open && setDeleteTarget(null)}>
        <DialogContent className="bg-background/95 backdrop-blur border-white/10">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-400"><AlertTriangle className="w-5 h-5" /> Deletar Pacote</DialogTitle>
            <DialogDescription>Este pacote não estará mais disponível para compra.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
