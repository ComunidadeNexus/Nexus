...
const emptyCategory = {
  name: "", slug: "", description: "", icon: "📌", color: "#8b5cf6",
  is_active: true, is_admin_only: false, is_premium_only: false, order_position: 0,
};
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {categories.map(cat => (
            <div key={cat.id} className="relative p-5 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/8 transition-all">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="w-3 h-3 rounded-full border-2 border-white/20 shrink-0 mt-1" style={{ backgroundColor: cat.color }} />
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {cat.is_active ? (
                  <Badge className="text-xs bg-emerald-500/20 text-emerald-400 border-emerald-500/30">Ativa</Badge>
                ) : (
                  <Badge className="text-xs bg-red-500/20 text-red-400 border-red-500/30">Inativa</Badge>
                )}
                {cat.is_admin_only && (
              ...
              <div className="flex gap-2">
                ...
                >
                  {cat.is_active ? <EyeOff className="w-3.5 h-3.5 text-orange-400" /> : <Eye className="w-3.5 h-3.5 text-emerald-400" />}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>Nome *</Label>
                <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="bg-white/5 border-white/10" placeholder="Ex: Tecnologia" />
              </div>
              <div className="space-y-1.5">
                <Label>Slug *</Label>
                <Input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value.toLowerCase().replace(/\s+/g, "-") }))} className="bg-white/5 border-white/10 font-mono" placeholder="tecnologia" />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label>Descrição</Label>
              <Textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className="bg-white/5 border-white/10 resize-none h-20" placeholder="Descrição da categoria..." />
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-1.5">
                <Label>Ícone (emoji)</Label>
                <Input value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} className="bg-white/5 border-white/10 text-center text-xl" placeholder="📌" />
              </div>
              <div className="space-y-1.5">
                <Label>Cor</Label>
                <Input type="color" value={form.color} onChange={e => setForm(f => ({ ...f, color: e.target.value }))} className="bg-white/5 border-white/10 h-10 cursor-pointer p-1" />
              </div>
              <div className="space-y-1.5">
                <Label>Posição</Label>
                <Input type="number" value={form.order_position} onChange={e => setForm(f => ({ ...f, order_position: Number(e.target.value) }))} className="bg-white/5 border-white/10" />
              </div>
            </div>
            <div className="space-y-3">
              {[
                { key: "is_active", label: "Ativa", desc: "Exibir para usuários" },
                { key: "is_admin_only", label: "Somente Admin", desc: "Apenas admins podem postar" },
                { key: "is_premium_only", label: "Somente Premium", desc: "Requer assinatura premium" },
              ].map(({ key, label, desc }) => (
      ...
      <Dialog open={!!deleteTarget} onOpenChange={open => !open && setDeleteTarget(null)}>
        <DialogContent className="bg-background/95 backdrop-blur border-white/10">
          ...
          <p className="text-sm text-muted-foreground">Tem certeza? Posts desta categoria não serão afetados, mas a categoria não estará mais disponível.</p>
          <DialogFooter>
