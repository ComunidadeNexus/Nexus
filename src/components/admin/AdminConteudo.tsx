...
import { FileText, Search, Eye, EyeOff, Pin, PinOff, Trash2, MessageCircle, Filter, AlertTriangle } from "lucide-react";
...
const AdminConteudo = () => {
  const { posts, loadingPosts, fetchPosts, toggleHidePost, togglePinPost, deletePost } = useAdminData();
  ...
  useEffect(() => { fetchPosts(); fetchComments(); }, []);
      ...
      {activeTab === "posts" && (
        <>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Select value={filterType} onValueChange={v => { const val = v === "all" ? "" : v; setFilterType(val); fetchPosts(search, val); }}>
            ...
            <Button onClick={() => fetchPosts(search, filterType)} className="bg-violet-600 hover:bg-violet-700">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 bg-white/5">
                    <th className="text-left px-4 py-3 text-muted-foreground font-medium">Conteúdo</th>
                    <th className="text-left px-4 py-3 text-muted-foreground font-medium">Status</th>
                    <th className="text-left px-4 py-3 text-muted-foreground font-medium">Likes</th>
                    <th className="text-left px-4 py-3 text-muted-foreground font-medium">Comentários</th>
                ...
                <tbody>
                  {loadingPosts ? (
                    ...
                    <tr><td colSpan={6} className="text-center text-muted-foreground py-12">Nenhum post encontrado</td></tr>
                  ) : posts.map(post => (
                    <tr key={post.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 flex-wrap">
                          ...
                          {!post.is_hidden && !post.is_pinned && !post.is_premium_only && (
                            <Badge className="text-xs bg-emerald-500/20 text-emerald-400 border-emerald-500/30">Normal</Badge>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 justify-end">
                          ...
                          >
                            {post.is_hidden ? <Eye className="w-4 h-4 text-emerald-400" /> : <EyeOff className="w-4 h-4 text-orange-400" />}
                          </Button>
                          <Button
                            size="icon"
                            variant="ghost"
                          ...
                          >
                            {post.is_pinned ? <PinOff className="w-4 h-4 text-amber-400" /> : <Pin className="w-4 h-4 text-amber-400" />}
                          </Button>
                          <Button
                            size="icon"
                            variant="ghost"
      ...
      {activeTab === "comments" && (
        <>
          <div className="flex gap-3">
            ...
            <Button onClick={() => fetchComments(commentSearch)} className="bg-violet-600 hover:bg-violet-700">
              <Search className="w-4 h-4 mr-2" /> Buscar
            </Button>
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loadingComments ? (
                    ...
                    <tr><td colSpan={5} className="text-center text-muted-foreground py-12">Nenhum comentário encontrado</td></tr>
                    ...
                    <tr key={comment.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {comment.is_hidden ? (
                          <Badge className="text-xs bg-red-500/20 text-red-400 border-red-500/30">Oculto</Badge>
                        ) : (
                          <Badge className="text-xs bg-emerald-500/20 text-emerald-400 border-emerald-500/30">Visível</Badge>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 justify-end">
                          ...
                          >
                            {comment.is_hidden ? <Eye className="w-4 h-4 text-emerald-400" /> : <EyeOff className="w-4 h-4 text-orange-400" />}
                          </Button>
                          <Button
                            size="icon"
                            variant="ghost"
        ...
        <DialogContent className="bg-background/95 backdrop-blur border-white/10">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => { if (deleteTarget) { deletePost(deleteTarget); setDeleteTarget(null); } }}
            >
