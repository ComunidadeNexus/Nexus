...
import { Hexagon, Search, BadgeCheck, Lock, Unlock, Trash2, Users, FileText, AlertTriangle } from "lucide-react";
...
const AdminNucleos = () => {
  ...
  useEffect(() => { fetchNucleos(); }, []);

  const fetchNucleos = async (q = "") => {
    setLoading(true);
    let query = supabase.from("nucleos").select("*").order("members_count", { ascending: false }).limit(100);
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="text-center text-muted-foreground py-12">Nenhum núcleo encontrado</td></tr>
              ) : nucleos.map(n => (
                <tr key={n.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      {n.is_private ? (
                        <Badge className="text-xs bg-orange-500/20 text-orange-400 border-orange-500/30">Privado</Badge>
                      ) : (
                        <Badge className="text-xs bg-emerald-500/20 text-emerald-400 border-emerald-500/30">Público</Badge>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1 justify-end">
                      ...
                      >
                        <BadgeCheck className={`w-4 h-4 ${n.is_verified ? "text-sky-400" : "text-muted-foreground"}`} />
                      </Button>
                      <Button
                        size="icon"
                        variant="ghost"
                      ...
                      >
                        {n.is_private ? <Unlock className="w-4 h-4 text-emerald-400" /> : <Lock className="w-4 h-4 text-orange-400" />}
                      </Button>
                      <Button
                        size="icon"
                        variant="ghost"
      ...
      <Dialog open={!!deleteTarget} onOpenChange={open => !open && setDeleteTarget(null)}>
        <DialogContent className="bg-background/95 backdrop-blur border-white/10">
          <DialogHeader>
            ...
            <DialogDescription>Esta ação é permanente. O núcleo e todos os seus dados serão removidos.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
