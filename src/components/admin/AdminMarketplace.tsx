  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={filterStatus} onValueChange={v => { const s = v === "all" ? "" : v; setFilterStatus(s); fetchListings(search, s); }}>
        ...
        <Button onClick={() => fetchListings(search, filterStatus)} className="bg-violet-600 hover:bg-violet-700">
              ...
              {loading ? (
                Array(6).fill(0).map((_, i) => (
                  <tr key={i} className="border-b border-white/5">
                    {Array(7).fill(0).map((_, j) => <td key={j} className="px-4 py-3"><Skeleton className="h-8 rounded" /></td>)}
                  </tr>
                ))
              ) : listings.length === 0 ? (
                <tr><td colSpan={7} className="text-center text-muted-foreground py-12">Nenhum anúncio encontrado</td></tr>
              ) : listings.map(l => {
                ...
                return (
                  <tr key={l.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="px-4 py-3 max-w-xs">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{l.description?.slice(0, 50)}...</p>
                    </td>
